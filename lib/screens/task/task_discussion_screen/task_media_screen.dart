...
class _TaskMediaScreenState extends State<TaskMediaScreen> {
  ...
  List<MediaModel> lastTaskMediaList = [];
    ...
    speechHelper = SpeechToTextHelper(
      onSpeechResultCallback: (result) {
        setState(() {
          ...
          mediaSearchQuery = result;
        });
        context
            .read<TaskMediaBloc>()
            .add(TaskSearchMedia(result, widget.id));
        Navigator.pop(context);
      },
    );
    _getFirstTimeUser();
    context.read<TaskMediaBloc>().add(TaskMediaList(id: widget.id));
            ...
            child: BlocConsumer<TaskMediaBloc, TaskMediaState>(
              listener: (context, state) {
                ...
                if (state is TaskMediaPaginated) {
                  lastTaskMediaList = state.TaskMediaList;
                  ...
                  Future.delayed(Duration(milliseconds: 500), () {
                    context.read<TaskMediaBloc>().add(TaskSilentMediaRefresh(id: widget.id));
                  });
              ...
              builder: (context, state) {
                print(
                    "Current state: $state, lastTaskMediaList length: ${lastTaskMediaList.length}");

                Widget buildTaskMediaList(
                  ...
                  return NotificationListener<ScrollNotification>(
                    onNotification: (scrollInfo) {
                      ...
                      if (!reachedMax &&
                        ...
                        isLoadingMore = true;
                        setState(() {});
                        // Add load more event if available
                        // context.read<TaskMediaBloc>().add(TaskLoadMoreMedia(widget.id));
                      }
                      return false;
                    },
                    child: context
                ...
                if (state is TaskMediaLoading) {
                  ...
                  return buildTaskMediaList(
                      state.TaskMediaList, state.hasReachedMax);
                } else if (state is TaskDownloadInProgress) {
                  return buildTaskMediaList(lastTaskMediaList, hasReachedMax);
                } else if (state is TaskMediaUploadSuccess ||
                    state is TaskMediaError ||
                    state is TaskDownloadSuccess ||
                    state is TaskDownloadFailure ||
                    state is TaskMediaDeleteSuccess) {
                  if (lastTaskMediaList.isNotEmpty) {
                    return buildTaskMediaList(lastTaskMediaList, hasReachedMax);
                  } else {
                    context
                        .read<TaskMediaBloc>()
                ...
                return lastTaskMediaList.isNotEmpty
                    ? buildTaskMediaList(lastTaskMediaList, hasReachedMax)
                    : const NotesShimmer();
              },
            ),
          ),
          ...
          try {
              ...
              builder: (context) {
                return AlertDialog(
                  ...
                  actions: [
                    TextButton(
                      onPressed: () {
                        print("User confirmed deletion - about to pop with true");
                    ...
                    TextButton(
                      onPressed: () {
                        print("User cancelled deletion - about to pop with false");
            ...
            if (result == true) {
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                setState(() {
                  taskMediaList.removeAt(index);
                });
                context
                    .read<TaskMediaBloc>()
                    .add(DeleteTaskMedia(id: media.id));
              });
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        InkWell(
                          onTap: () {
                            context.read<TaskMediaBloc>().add(TaskStartDownload(
