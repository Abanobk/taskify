...
class SubTaskScreen extends StatefulWidget {
  final int taskId;
  final String from;
  const SubTaskScreen({super.key, required this.taskId, required this.from});
...
class _SubTaskScreenState extends State<SubTaskScreen>
  ...
  void initState() {
    ...
    widget.from == "projectTask"
        ? BlocProvider.of<TaskBloc>(context)
            .add(AllTaskListOnTask(id: widget.taskId, isSubtask: false))
        : BlocProvider.of<TaskBloc>(context)
            .add(AllTaskListOnTask(id: widget.taskId, isSubtask: true));
    super.initState();
    getIsFirst();
  }
  ...
  void onDeleteTask(task) {
    ...
    widget.from == "projectTask"
        ? BlocProvider.of<TaskBloc>(context).add(
            AllTaskListOnTask(projectId: [widget.taskId], isSubtask: false))
        : BlocProvider.of<TaskBloc>(context)
            .add(AllTaskListOnTask(id: widget.taskId, isSubtask: true));
  }
    ...
    return Expanded(
      child: RefreshIndicator(
        ...
        child: (context.read<PermissionsBloc>().isManageTask == true)
            ? FutureBuilder(
                future:
                    Future.delayed(Duration(seconds: 1)), // Delay by 2 seconds
            ...
            if (direction == DismissDirection.endToStart) {
              ...
              widget.from == "projectTask"
                  ? BlocProvider.of<TaskBloc>(context).add(AllTaskListOnTask(
                      projectId: [widget.taskId], isSubtask: false))
                  : BlocProvider.of<TaskBloc>(context).add(
                      AllTaskListOnTask(id: widget.taskId, isSubtask: true));
