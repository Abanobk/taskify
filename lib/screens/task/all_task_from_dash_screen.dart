...
class _AllTaskScreenState extends State<AllTaskScreen>
        ...
        child: (context.read<PermissionsBloc>().isManageTask == true)
          ...
          listener: (context, state) {
            if (state is TaskPaginated) {
              print("TaskPaginated: ${state.task.length} tasks, hasReachedMax: ${state.hasReachedMax}");
            }
          },
          builder: (context, state) {
            print("TaskBloc Builder: $state");
            if (state is TaskInitial || state is TaskLoading || state is TodaysTaskList|| state is TodaysTaskLoading) {
              return const NotesShimmer();
            } else if (state is TaskPaginated) {
              if (state.isToday == true) {
                return const NotesShimmer(); // Ignore today's tasks
              }
              if (state.task.isEmpty && !state.hasReachedMax) {
                return const NotesShimmer(); // Keep showing shimmer if more data might be available
              } else if (state.task.isEmpty) {
                return NoData(isImage: true); // Show NoData only when no tasks and no more data
              }
                    ...
                    if (index < state.task.length) {
                      ...
                      return index == 0
                          ...
                          child: _listOfProject(
                              task, isLightTheme, date, state.task, index, state.task[index]))
                          : _listOfProject(
                          task, isLightTheme, date, state.task, index, state.task[index]);
                    } else {
                      return Padding(
            ...
            return const NotesShimmer(); // Fallback for unhandled states
          },
        )
            : NoPermission(),
      ),
  ...
  Widget _listOfProject(task, isLightTheme, date, statetask, index, taskModel) {
    return Padding(
        ...
        child: DismissibleCard(
          ...
          confirmDismiss: (DismissDirection direction) async {
            if (direction == DismissDirection.endToStart) {
              ...
              print("ugbhn ${task.customFieldValues}");
                  ...
                  children: [
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          Expanded(
                            // Replace Container(width: double.infinity)
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Row(
                    ...
                    Padding(
                      // Replace SizedBox with width: 300.w
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  task.users!.isEmpty
                                      ...
                                      : SizedBox(
                                          width: 20
                                              .w), // Reduced from 40.w to 20.w for better spacing
