...
print("fhuodfx ${event.recurringFrequencyType}");
  ...
  Future<void> _getAllTaskListOnProject(AllTaskListOnTask event, Emitter<TaskState> emit) async {
    try {
      List<Tasks> task = [];
      _offset = 0; // Reset offset for the initial load
      _hasReachedMax = false;
      ...
      _offset += _limit;
      _hasReachedMax = task.length >= result['total']; // Use total count from API
  ...
  Future<void> _onTodaysTask(TodaysTaskList event, Emitter<TaskState> emit) async {
    try {
      List<Tasks> task = [];
      _offset = 0; // Reset offset for the initial load
      _hasReachedMax = false;
      ...
      // Fixed: Check if we've reached max based on fetched items vs limit
      _hasReachedMax = task.length < _limit;
      // Alternative: if you want to use total count, do this instead:
      // _hasReachedMax = task.length >= result['total'];
